...
const deleteKeyPoint = async (req, res) => {
    ...
    try {
        await keyPoint.deleteOne({ _id: inputs._id });
        res.status(200).json({ message: "Punto clave borrado correctamente" });
    } catch (error) {
        res.status(400).json({ message: "No se encontró el punto clave o no se pudo eliminar" });
    }
}

module.exports = deleteKeyPoint;