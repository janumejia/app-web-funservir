...
const { ...regex } = require("../../../regex.js") // Importación de patrones de Regex
...
const deleteComment = async (req, res) => {
  ...
  try {
    const updatedComment = await Comment.findByIdAndUpdate(
      inputs._id,
      {
        $set: { isReported: false, reportedBy: [] },
      },
    ...
    if (updatedComment) {
      ...
      res.status(404).json({ message: "No se pudo actualizar el comentario" });
    }
  } catch (error) {
