...
import { FormHeader, Title, FormContent, FormAction } from './AddCreateKeyPoint.style';
// const { Option } = Select;
...
const SiteLocation = ({ setStep, availableLocalities, availableNeighborhoods }) => {
  ...
  const aux1 = [];
  const imgsCloudinary = state?.dataAddSite?.sitePhotos?.map(async (img) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <FormContent>
        ...
        >
          <Controller
            name="siteAddress"
            defaultValue={state?.dataAddSite?.siteAddress}
            control={control}
            rules={{
              required: true,
        ...
        >
          <Controller
            name="locality"
            defaultValue={state?.dataAddSite?.locality}
            control={control}
            rules={{
              required: true,
        ...
        >
          <Controller
            name="neighborhood"
            defaultValue={state?.dataAddSite?.neighborhood}
            control={control}
            rules={{
              required: true,
              // pattern: /^[a-zA-Z0-9 #,.-]{5,255}$/, // Como es de selección multiple no es necesario el regex
