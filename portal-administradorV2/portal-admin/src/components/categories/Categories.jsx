...
const ManageCategories = () => {
    ...
    const saveEdit = async (key) => {
        try {
            ...
            if (key === "0" && row.name) { // Si el campo nombre tiene información...
                ...
                axios.post('/addCategory', newLocation, { headers: { 'token': localStorage.getItem("token") } })
                ...
                axios.post('/editCategory', { ...item, ...row, ...mVals }, { headers: { 'token': localStorage.getItem("token") } })
                    .then((res) => { // Aquí se manejan los códigos de respuesta buenas (200 - 399)
    ...
    const handleDelete = (key) => {
        ...
        const index = newData.findIndex((item) => key === item._id);
        if (key !== "0") {
            axios.post('/deleteCategory', { _id: key }, { headers: { 'token': localStorage.getItem("token") } })
                .then((res) => { // Aquí se manejan los códigos de respuesta buenas (200 - 399)
